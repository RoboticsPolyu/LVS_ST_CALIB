...
int main(int argc, char **argv) 
{
    ...
    float pl_end_dx = 0, pl_end_dy = -0.50, pl_end_dz = 0;
    ...
    opt << 0 << " " << dt << " " << new_Lw(0) << " " << new_Lw(1) << " " << new_Lw(2) << " " << new_Lw(3) << " " << 0 << " " << 
    ...
    for(uint iter_num = 0; iter_num < ITER_NUM; iter_num++)
    {
        ...
        for(uint idx = 0; idx < uv_lens-2; idx = idx + 10)
        {
            ...
            if(error.norm() < ERR_THR)
            {
                ...
                distance_2_sum += error.dot(error);
            }
            ...
            // std::cout << "---> BASE_CORNER: [ " << base_P_corner[0] << " " << base_P_corner[1] << " " << base_P_corner[2] << " " << base_P_corner[3] << " ] <---" << std::endl;
                laser_point_file << base_P_corner[0] << " " << base_P_corner[1] << " " << base_P_corner[2] << " " << base_P_corner[3] << " " << error.norm() << std::endl;
            }
        ...
        if(enable_ext_opt)
        {
            ...
            dP = - (J_P.transpose()* J_P + miu_p* gtsam::I_3x3).inverse()* J_P.transpose()* J_error;
