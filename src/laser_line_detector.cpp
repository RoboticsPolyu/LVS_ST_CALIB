...
void  LineFinder::drawfittingLines(cv::Mat& fittingline, cv::Vec4f liness1)
{
	...
	int x0 = liness1[2]; 
	int y0 = liness1[3];
	int x1 = x0 + 2500 * liness1[0];
	int y1 = y0 + 2500 * liness1[1];
	int x2 = x0 - 2500 * liness1[0]; 
	int y2 = y0 - 2500 * liness1[1]; 

	cv::line(fittingline, cv::Point(x2, y2), cv::Point(x1, y1),
		color1); // ÑÕÉ«ºÍ¿í¶È
}

double LineFinder::least_method(std::vector<cv::Point> points)   

{
	...
	for (int k = 0; k < n; k++)
	{
		i = points[k].x;
		j = points[k].y;
		minex = minex + (a0 + a1 * i - j) * (a0 + a1 * i - j);
	}
	return minex;
}
	...
	for (int i = 0; i < img.rows; i++)
	{
		for (int j = 0; j < img.cols; j++)
		{
			...
			if (k > 100)
			{
				...
				rect1 &= Rect(0, 0, img.cols, img.rows);
