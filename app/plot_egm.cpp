...
// string trajectory_file = "/home/yang/works/wcq_lab/build/pattern_pose_file.txt"; // 
string trajectory_file = "/home/yang/works/demo_Laser_welding1/build/dataset/handeye_31/info/egm_data.txt";
string matlab_tra_file = "/home/yang/Downloads/camera_calibration.txt";
...
{   
  ...
  while (!fin.eof()) 
  {
    ...
    double eularx, eulary, eularz;
    fin >> time >> tx >> ty >> tz >> eularx >> eulary >> eularz >> qw >> qx >> qy >> qz;
    std::cout << index << " " << tx << " " << ty << " " << tz << " " << eularx << std::endl;
    index++;
    Quaterniond q(qw, qx, qy, qz);

    //Quaterniond q = gtsam::Quaternion(gtsam::Rot3::Expmap(gtsam::Vector3(eularx/180.0* M_PI, eulary/180.0* M_PI, eularz/180.0* M_PI)).matrix());
    // Quaterniond q = Eigen::AngleAxisd(eularz/180.0* M_PI, ::Eigen::Vector3d::UnitZ()) *Eigen::AngleAxisd(eulary/180.0* M_PI, ::Eigen::Vector3d::UnitY()) *Eigen::AngleAxisd(eularx/180.0* M_PI, ::Eigen::Vector3d::UnitX());
    // Quaterniond q = Eigen::AngleAxisd(eularz, ::Eigen::Vector3d::UnitZ()) *Eigen::AngleAxisd(eulary, ::Eigen::Vector3d::UnitY()) *Eigen::AngleAxisd(eularx, ::Eigen::Vector3d::UnitX());

    // fin >> time >> tx >> ty >> tz >> qx >> qy >> qz >> qw;
 
    // 用Eigen::Isometry表示欧氏变换矩阵
    Eigen::Isometry3d Twr = Eigen::Isometry3d::Identity( );  // 三维变换矩阵
    gtsam::Vector3 rot(-0.50823015332, 0.48478994671, -1.47728977528);
    // Twr.rotate(q* gtsam::Rot3::Expmap(rot).matrix()); // Quaterniond(qw, qx, qy, qz) );  // 旋转部分赋值
  ...
  // {
  //   Eigen::Isometry3d new_pose = poses[i];
  //   new_pose.rotate(new_pose.rotation()* gtsam::Rot3::Expmap(rot).matrix());
  //   poses[i] = new_pose;
  // }
...
void DrawTrajectory(vector<Isometry3d, Eigen::aligned_allocator<Isometry3d>> poses) {
  ...
  pangolin::OpenGlRenderState s_cam(
    ...
    pangolin::ModelViewLookAt(poses[0].translation().x(), poses[0].translation().y(), poses[0].translation().z(), 
      poses[0].translation().x() + 10, poses[0].translation().y(), poses[0].translation().z(), 0.0, -1.0, 0.0) //摄像机初始时刻所处的位置 摄像机的视点位置（即摄像机的光轴朝向哪一个点）以及摄像机的本身哪一轴朝上
  );
